...
>
> 3D 미리보기는 **큐브 지오메트리 + 본 계층**만 그리는 *근사*입니다(텍스처·메시·Molang·IK·애니메이션 제외).
> 데미지 포인트를 모델 크기·방향에 맞춰 배치하는 **공간 기준**이 목적이며, 최종 룩은 인게임 BetterModel이 기준입니다.
...
- **3D Preview 탭**: 불러온 `.bbmodel`을 3D로 렌더링하고 **데미지 볼륨(구/박스/콘)을 모델 위에 겹쳐** 표시.
  좌드래그 궤도 회전, 스크롤 줌. 편집 시 실시간 갱신 → 모델을 보면서 포인트 위치를 맞출 수 있음
- 파일 New/Open/Save/Save As, 변경(dirty) 표시
- 향후: **애니메이션 재생**(키프레임 샘플링), 3D 뷰포트 내 직접 드래그, 아이템/블록 키 자동완성
